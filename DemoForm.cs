...
foreach (Match cm in Regex.Matches(t, @"\bclass\s+(\w+)")) used.Add(cm.Groups[1].Value);
}
}
...
string folder = _folderText.Text.Trim();
string projRoot = FindProjectRoot(folder);
if (projRoot != null)
{
foreach (string cf in Directory.GetFiles(projRoot, "*.cs", SearchOption.AllDirectories))
{
if (cf.Contains("\\bin\\") || cf.Contains("\\obj\\")) continue;
string t3;
try { t3 = File.ReadAllText(cf); } catch { continue; }
if (Regex.IsMatch(t3, @"\bclass\s+" + Regex.Escape(newName) + @"\b"))
{
Log("Конфликт: класс " + newName + " уже существует в проекте (" + cf + ")");
MessageBox.Show(this, "Класс с именем " + newName + " уже есть в проекте:" + Environment.NewLine + cf + Environment.NewLine + Environment.NewLine + "Два одинаковых класса в одной namespace не скомпилируются. Выбери другое имя.", "Конфликт имён", MessageBoxButtons.OK, MessageBoxIcon.Warning);
return;
}
}
}
...
if (!string.Equals(Path.GetDirectoryName(extra), Path.GetDirectoryName(src.Path), StringComparison.OrdinalIgnoreCase)) continue;
string t2;
...
string srcResx = Path.ChangeExtension(src.Path, ".resx");
if (File.Exists(srcResx))
{
string destResx = Path.Combine(dest, newName + ".resx");
File.Copy(srcResx, destResx, true);
created.Add(destResx);
Log("Скопирован ресурс: " + destResx);
}
